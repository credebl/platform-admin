...
export default function AppLauncher() {
  ...
  const currentApp: string = (process.env.NEXT_PUBLIC_APP_NAME || "").toUpperCase();

  const selectedOrg = useAppSelector(
    (state) => state.organization.selectedOrganization
  );
  const [appLaunchDetails, setAppLaunchDetails] = useState<AppLauncherInterface[]>([]);

  const getAppLink = (app: AppLauncherInterface) => {
    switch (env) {
      case "LOCAL":
  ...
  useEffect(() => {
    if (Array.isArray(selectedOrg?.appLaunchDetails) && selectedOrg.appLaunchDetails.length > 0) {
      setAppLaunchDetails(selectedOrg.appLaunchDetails as AppLauncherInterface[]);
    } else {
      setAppLaunchDetails(appsData as AppLauncherInterface[]);
    }
  }, [selectedOrg]);

  const filteredApps = appLaunchDetails.filter(
    ...
    <Popover>
      ...
      >
        ...
        <div className="grid grid-cols-2 place-items-center">
          {filteredApps.map((app: AppLauncherInterface) => (
            <a
              key={app.name}
              href={getAppLink(app)}
              target="_blank"
            ...
            >
              ...
              />
              <span className="mt-2 text-md font-medium">{app.name}</span>
            </a>
          ))}
        </div>
      </PopoverContent>
