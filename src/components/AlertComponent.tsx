import { IAlertComponent } from '@/utils/common.interfaces'
...
export const AlertComponent = ({
  ...
  const getAlertClass = (): string => {
    switch (type) {
      case 'warning':
        return 'text-foreground badges-warning bg-yellow-100'
      case 'failure':
        return 'text-foreground badges-error bg-red-100'
      case 'success':
        return 'text-foreground badges-success bg-green-100'
      default:
  ...
  return (
    message !== null && (
      <div className="w-full">
        <div
          className={`mb-4 border flex flex-col gap-2 rounded-lg p-4 text-sm ${getAlertClass()}`}
          role="alert"
        >
          <div
